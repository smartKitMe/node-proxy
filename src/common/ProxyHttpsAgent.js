var AgentOrigin = require('agentkeepalive').HttpsAgent;
const colors = require('colors');

/**
 * 优化的 HTTPS 代理 Agent 类
 * 基于 agentkeepalive.HttpsAgent 实现高性能的 HTTPS 连接池管理
 * 支持详细的性能监控和调试日志
 */
module.exports = class Agent extends AgentOrigin {
    constructor(options = {}) {
        ...
        const defaultOptions = {
            ...
            // HTTPS 特有配置
            rejectUnauthorized: false,      // 允许自签名证书（代理场景常用）
            secureProtocol: 'TLS_method',   // 使用最新的 TLS 协议
            ...options
        };
        ...
        if (this.enableDebugLogs) {
            console.log(colors.cyan('[ProxyHttpsAgent] 初始化完成，配置:'), {
    ...
    _setupEventListeners() {
        ...
        this.on('socket', (socket, options) => {
            ...
            if (socket._reused) {
                ...
                if (this.enableDebugLogs) {
                    console.log(colors.green(`[ProxyHttpsAgent] 复用 HTTPS 连接: ${options.host}:${options.port}`));
                }
            } else {
                this.stats.newConnections++;
                this.stats.activeConnections++;
                if (this.enableDebugLogs) {
                    console.log(colors.yellow(`[ProxyHttpsAgent] 新建 HTTPS 连接: ${options.host}:${options.port}`));
                }
            }
            
            // 监听 SSL/TLS 握手事件
            if (socket.on && !socket._reused) {
                socket.on('secureConnect', () => {
                    this.stats.sslHandshakes++;
                    if (this.enableDebugLogs) {
                        const cert = socket.getPeerCertificate();
                        console.log(colors.blue(`[ProxyHttpsAgent] SSL 握手完成: ${options.host}:${options.port}`), {
            ...
            socket.on('close', () => {
                ...
                if (this.enableDebugLogs) {
                    console.log(colors.gray(`[ProxyHttpsAgent] HTTPS 连接关闭: ${options.host}:${options.port}`));
                }
            });
            ...
            socket.on('timeout', () => {
                this.stats.timeouts++;
                if (this.enableDebugLogs) {
                    console.log(colors.red(`[ProxyHttpsAgent] HTTPS 连接超时: ${options.host}:${options.port}`));
                }
            });
            ...
            socket.on('error', (err) => {
                ...
                if (err.code === 'CERT_HAS_EXPIRED' || err.code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE') {
                    this.stats.sslErrors++;
                }
                if (this.enableDebugLogs) {
                    console.log(colors.red(`[ProxyHttpsAgent] HTTPS 连接错误: ${options.host}:${options.port}, ${err.message}`));
                }
            });
        });
    }
    ...
    _startPerformanceReporting() {
        this.performanceInterval = setInterval(() => {
            ...
            console.log(colors.cyan('[ProxyHttpsAgent] HTTPS 性能统计:'), {
                总请求数: stats.totalRequests,
                活跃连接: stats.activeConnections,
                连接复用率: `${stats.reuseRate}%`,
                SSL握手次数: stats.sslHandshakes,
                SSL错误率: `${stats.sslErrorRate}%`,
    ...
    getName(option) {
        ...
        if (option.customSocketId) {
            ...
            if (this.enableDebugLogs) {
                console.log(colors.blue(`[ProxyHttpsAgent] NTLM Socket ID: ${option.customSocketId}`));
            }
        }
        return name;
    }
    ...
    getPerformanceStats() {
        ...
        const sslErrorRate = this.stats.sslHandshakes > 0 ? ((this.stats.sslErrors / this.stats.sslHandshakes) * 100).toFixed(2) : 0;
        ...
        return {
            ...
            sslHandshakes: this.stats.sslHandshakes,
            sslErrors: this.stats.sslErrors,
            sslErrorRate: parseFloat(sslErrorRate),
    ...
    resetStats() {
        ...
        if (this.enableDebugLogs) {
            console.log(colors.cyan('[ProxyHttpsAgent] HTTPS 性能统计已重置'));
        }
    }
    ...
    destroy() {
        ...
        if (this.enableDebugLogs) {
            console.log(colors.cyan('[ProxyHttpsAgent] HTTPS Agent 实例已销毁'));
        }
